...
void bst_print(RBTPtr self, NodePtr tree, int level) {
...
void main() {
	...
	printf("The entire number of RBNode: %d \n", Node_num);

	printf("The number of BlackNode: %d \n", HowManyBN(rbt, rbt->r));
	printf("The height of BlackNode: %d \n", BlackNode_Height(rbt, rbt->r));

	inorder_traversal(rbt, rbt->r);
	bst_print(rbt, rbt->r, 0);
	return;
}